...
export const CodeEditor = forwardRef<CodeEditorHandle, Props>(
  function CodeEditor(
    ...
    const updateAutocomplete = useCallback((val: string, cursorPos: number) => {
    ...
    return (
      <div
        className={`relative flex h-full w-full overflow-hidden bg-card ${editorFont ? "" : "font-mono"}`}
        style={{
          fontSize: `${fontSize}px`,
          lineHeight: `${lh}px`,
          ...(editorFont ? { fontFamily: editorFont } : {}),
        }}
      >
        {/* gutter */}
        <div
              ...
              __html:
                lines
                  .map((line, idx) => {
                    const lineNum = idx + 1;
                    const varForLine =
                      highlightVariable?.line === lineNum
