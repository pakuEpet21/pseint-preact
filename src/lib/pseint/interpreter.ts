...
class Parser {
    ...
    while (this.peek().type !== "eof") {
      if (this.peek().type === "newline") {
        ...
        if (
          ...
          syncKeywords.has(this.peek().value.toLowerCase())
        ) {
          return;
        }
      } else {
      ...
      switch (kw) {
        ...
        case "esperar": {
          ...
          if (
            ...
            (next.value.toLowerCase() === "segundos" ||
              next.value.toLowerCase() === "segundo")
          )
            this.next();
  ...
  peekAfterValueIsColon(): boolean {
    ...
    let p = this.pos;
    const at = (o: number) =>
      this.tokens[Math.min(p + o, this.tokens.length - 1)];
    let k = 0;
...
class Interpreter {
  ...
  async execStmt(node: Node, scope: Scope): Promise<any> {
    ...
    switch (node.type) {
      ...
      case "Wait": {
        const totalMs =
          this.toNum(await this.evalAsync(node.expr, scope)) * 1000;
      ...
      case "Dimension":
        for (const d of node.decls) {
          const dims = d.dims.map((e: Node) =>
            Math.floor(this.toNum(this.eval(e, scope))),
          );
      ...
      case "For": {
        ...
        const step = node.step
          ? this.toNum(await this.evalAsync(node.step, scope))
          : 1;
      ...
      default:
        throw new PseintError(
          `Sentencia desconocida: ${node.type}`,
          node.line || 0,
        );
    }
  }
  ...
  assign(target: Node, value: Value, scope: Scope) {
    if (target.indices && target.indices.length > 0) {
      ...
      if (!Array.isArray(v.value))
        throw new PseintError(
          `"${target.name}" no es un arreglo`,
          target.line || 0,
        );
      const idx = target.indices.map((e: Node) =>
        Math.floor(this.toNum(this.eval(e, scope))),
      );
      const checked = this.coerceToType(
        value,
        v.declaredType,
        target.name,
        target.line || 0,
  ...
  evalSync(node: Node, scope: Scope): Value {
    switch (node.type) {
      ...
      case "Unary": {
        if (node.op === "-")
          return -this.toNum(this.evalSync(node.expr, scope));
        if (node.op === "no")
          return !this.toBool(this.evalSync(node.expr, scope));
      ...
      case "Binary":
        return this.binary(
          node,
          this.evalSync(node.left, scope),
          this.evalSync(node.right, scope),
        );
      case "Call":
  ...
  async evalAsync(node: Node, scope: Scope): Promise<Value> {
    switch (node.type) {
      ...
      case "Unary": {
        if (node.op === "-")
          return -this.toNum(await this.evalAsync(node.expr, scope));
        if (node.op === "no")
          return !this.toBool(await this.evalAsync(node.expr, scope));
  ...
  readVar(node: Node, scope: Scope, _async: boolean): Value {
    ...
    if (!v)
      throw new PseintError(
        `La variable "${node.name}" no está definida`,
        node.line || 0,
      );
  ...
  binary(node: Node, l: Value, r: Value): Value {
    ...
    switch (op) {
      ...
      case "div": {
        ...
        if (d === 0)
          throw new PseintError(
            "División entera entre cero (DIV)",
            node.line || 0,
          );
  ...
  async callFunction(node: Node, scope: Scope): Promise<Value> {
    ...
    if (fn) {
      ...
      for (let k = 0; k < fn.params.length; k++) {
        const arg = node.args[k]
          ? await this.evalAsync(node.args[k], scope)
          : 0;
    ...
    const args = await Promise.all(
      node.args.map((a: Node) => this.evalAsync(a, scope)),
    );
...
function normalizeType(type: string): string | undefined {
  ...
  if (["caracter", "caracteres", "texto", "cadena"].includes(t))
    return "caracter";
  return undefined;
}
...
function buildKeywordHint(expected: string, found: Token): string | undefined {
  ...
  if (
    (expected === "hacer" || expected === "finmientras") &&
    foundVal === "entonces"
  ) {
...
export async function runPseint(
  source: string,
  opts: RunOptions,
): Promise<void> {
