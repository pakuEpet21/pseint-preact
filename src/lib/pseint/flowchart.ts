...
function layoutBlock(stmts: any[], startX: number, startY: number, prevNode?: FlowNode): LayoutResult {
  ...
  for (const stmt of stmts) {
    ...
    switch (stmt.type) {
      ...
      case "Switch": {
        ...
        let currentY = y
        const caseColors = [COLORS.arrowTrue, COLORS.arrowFalse, COLORS.arrowLoop, "#f59e0b", "#3b82f6"]
        ...
        if (stmt.defaultBody?.length) {
          const defX = startX + ((stmt.cases || []).length % 2 === 0 ? -1 : 1) * BRANCH_OFFSET_X
...
function computeViewBox(nodes: FlowNode[], arrows: FlowArrow[], padding: number) {
  const xs: number[] = []
  const ys: number[] = []
  ...
  arrows.forEach((a) => {
    ...
    if (a.via) a.via.forEach((v) => { xs.push(v.x); ys.push(v.y) })
  })
  ...
  return { offsetX, minX: 0, minY: minY - padding, maxX: targetWidth, maxY: maxY + padding }
}
...
export function generateFlowchart(source: string): FlowchartResult {
  ...
  allNodes.forEach((n) => { n.x += vb.offsetX })
  allArrows.forEach((a) => {
    a.fromX += vb.offsetX
    a.toX += vb.offsetX
    if (a.via) a.via.forEach((v) => { v.x += vb.offsetX })
  })
